...
const SummaryCard: React.FC<{ label: string; value: number; color: string; icon: React.ReactNode }> = ({ label, value, color, icon }) => (
    <div className="bg-slate-900/80 p-8 rounded-[2rem] shadow-premium border border-slate-800 group hover:border-slate-700 transition-all">
        <div className="flex items-center justify-between mb-6">
            <span className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em]">{label}</span>
            <div className={`p-4 rounded-2xl bg-slate-800 text-primary-400 border border-slate-700 group-hover:scale-110 transition-transform`}>
                {icon}
            </div>
        </div>
...
export const Reports: React.FC<ReportsProps> = ({ transactions, categories }) => {
    ...
    return (
        <div className="space-y-12 pb-24">
            ...
            <div className="bg-slate-900/50 backdrop-blur-md p-6 rounded-[2.5rem] shadow-premium flex flex-col lg:flex-row justify-between items-center gap-8 border border-slate-800">
            ...
            {filteredTransactions.length > 0 ? (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        ...
                        <div className="bg-gradient-to-br from-slate-900 to-black p-8 rounded-[2rem] shadow-premium border border-luxury-gold/30 flex flex-col justify-between">
                            <p className="text-[10px] font-black uppercase text-luxury-gold tracking-[0.2em] mb-4">Tỷ lệ tiết kiệm</p>
                            <div className="text-5xl font-black text-white font-mono tracking-tighter">{stats.savingsRate}%</div>
                    ...
                    {/* Trend Chart */}
                    <div className="bg-slate-900/90 p-12 rounded-[3rem] shadow-premium border border-slate-800">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        <div className="space-y-10">
                            <div className="bg-slate-900/90 p-10 rounded-[3rem] shadow-premium border border-slate-800">
                                <h3 className="text-[14px] font-black uppercase tracking-[0.3em] mb-12 text-rose-500 flex items-center">
                            ...
                            <div className="bg-slate-900/90 p-10 rounded-[3rem] shadow-premium border border-slate-800">
                                <h3 className="text-[14px] font-black uppercase tracking-[0.3em] mb-12 text-luxury-gold flex items-center">
                        ...
                        <div className="space-y-10">
                            <div className="bg-slate-900/90 p-10 rounded-[3rem] shadow-premium border border-slate-800">
                                <h3 className="text-[14px] font-black uppercase tracking-[0.3em] mb-12 text-emerald-500 flex items-center">
