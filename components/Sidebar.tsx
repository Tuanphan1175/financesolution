...
const NavItem: React.FC<NavItemProps> = ({ label, icon, isActive, onClick }) => {
  const iconWithClass = React.cloneElement(icon, { className: "h-6 w-6" });
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const STORAGE_KEY = "smartfinance_username";
  ...
  const navItems = useMemo<Array<{ view: View; label: string; icon: IconEl }>>(
    () => [
  ...
  return (
    <div className={`flex flex-col w-full h-full px-6 py-12 bg-luxury-obsidian overflow-y-auto ${className ?? ""}`}>
      {/* LOGO */}
      <div
      ...
      >
        ...
        <div className="ml-5">
          <h2 className="text-xl font-black text-white tracking-[0.2em] italic">TÀI CHÍNH</h2>
          <span className="text-[10px] font-black text-luxury-gold tracking-[0.55em]">PREMIUM</span>
        </div>
      </div>
          ...
          <div className="ml-5 flex-1 min-w-0">
            {isEditing ? (
              ...
              >
                ...
                <p className="text-[9px] font-black text-luxury-gold truncate uppercase tracking-[0.3em] opacity-60">
                  Elite Member
                </p>
              </div>
            )}
