        ...
        <tr className="hover:bg-slate-800/40 transition-all group border-b border-slate-800/50">
            <td className="px-8 py-6 whitespace-nowrap">
                <div className="flex items-center">
                    <div className="p-4 rounded-2xl shadow-lg border border-white/5 group-hover:scale-110 transition-transform" style={{ backgroundColor: `${category?.color || '#9ca3af'}20` }}>
                         <IconComponent className="h-6 w-6" style={{ color: category?.color || '#9ca3af' }} />
                    </div>
                    <div className="ml-5">
                        <div className="text-[15px] font-black text-white tracking-tight leading-none mb-2">{transaction.description}</div>
                        <div className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{category?.name || 'Khác'}</div>
                    </div>
                </div>
            </td>
            ...
            <td className={`px-8 py-6 whitespace-nowrap text-right ${isIncome ? 'text-emerald-400' : 'text-rose-400'}`}>
                <div className="text-xl font-black font-mono tracking-tighter">{isIncome ? '+' : '-'}{transaction.amount.toLocaleString('vi-VN')} ₫</div>
...
export const Transactions: React.FC<TransactionsProps> = ({ transactions, setTransactions, categories, onAddCategory, onUpdateCategory }) => {
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full">
                                <thead className="bg-black/20 border-b border-slate-800">
                                    <tr>
                                        <th className="px-8 py-5 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Nội dung</th>
                                        <th className="px-8 py-5 text-center text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Phân loại</th>
                                        <th className="px-8 py-5 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Ngày tháng</th>
                                        <th className="px-8 py-5 text-right text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Dòng tiền (VND)</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-800/50">
                                    {filteredTransactions.length > 0 ? (
                                        filteredTransactions.map(t => <TransactionRow key={t.id} transaction={t} categories={categories} />)
                                    ) : (
                                        <tr>
                                            <td colSpan={4} className="text-center text-slate-600 font-bold py-32 italic uppercase tracking-widest">
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-black p-8 rounded-[2.5rem] border border-luxury-gold/20 shadow-luxury">
                        ...
                        <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            {categories.map(cat => {
                                ...
                                return (
                                    <div key={cat.id} className="flex items-center justify-between p-4 rounded-2xl bg-black/30 border border-slate-800 hover:border-slate-700 group transition-all">
                                        <div className="flex items-center">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{cat.type === 'income' ? 'Thu nhập' : 'Chi tiêu'}</p>
                                            </div>
                                        </div>
                                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button className="text-slate-600 hover:text-luxury-gold p-1"><CogIcon className="w-4 h-4" /></button>
                                        </div>
                                    </div>
                                );
                            })}
